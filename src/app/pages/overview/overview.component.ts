...
export class OverviewComponent implements OnInit, OnDestroy {
    ...
    // Most Recent Sign Up Variable
    recentSignUp = '';

    // Payments and Late Fee Variables
    payments = 0;
    lateFeeDate = new Date('Tue Jul 2 2019 00:00:00 GMT-0400 (Eastern Daylight Time)');

    // Bar Chart Initial Data
    public barChartOptions = {
        scaleShowVerticalLines: false,
        responsive: true
    };
    public barChartLabels = ['Before June', 'June 1-7', 'June 8-14', 'June 15-21', 'June 22-28',
        'June 29-July 5', 'July 6-12', 'July 13-19'];
    public barChartType = 'bar';
    public barChartLegend = false;
    public barChartData = [
        { data: [0, 0, 0, 0, 0, 0, 0, 0, 0], label: 'Register' },
    ];
    ...
    loadAttendees() {
        this.firestoreService.getAttendees().pipe(
            ...
            this.calculateStatistics(attendees);
        });
    }

    calculateStatistics(attendees: Array<Attendee>) {

        // ----------SECTION ONE: VARIABLE INITIALIZATION-----------------

        // Calculate Number of Attendees
        this.numberOfAttendees = attendees.length;

        // Set Bar Chart Variables
        const barChartData = [0, 0, 0, 0, 0, 0, 0, 0];
        const dateOne = new Date('Sat Jun 1 2019 00:00:00 GMT-0400 (Eastern Daylight Time)');
        const dateTwo = new Date('Sat Jun 8 2019 00:00:00 GMT-0400 (Eastern Daylight Time)');
        const dateThree = new Date('Sat Jun 15 2019 00:00:00 GMT-0400 (Eastern Daylight Time)');
        const dateFour = new Date('Sat Jun 22 2019 00:00:00 GMT-0400 (Eastern Daylight Time)');
        const dateFive = new Date('Sat Jun 29 2019 00:00:00 GMT-0400 (Eastern Daylight Time)');
        const dateSix = new Date('Sat Jul 6 2019 00:00:00 GMT-0400 (Eastern Daylight Time)');
        const dateSeven = new Date('Sat July 13 2019 00:00:00 GMT-0400 (Eastern Daylight Time)');
        const dateEight = new Date('Sat Jun 20 2019 00:00:00 GMT-0400 (Eastern Daylight Time)');

        // Set Age Group Variables
        this.ageGroupOne = 0;
        this.ageGroupTwo = 0;
        this.ageGroupThree = 0;
        this.ageGroupFour = 0;

        // Set Recent Sign Up Variable
        this.recentSignUp = '';

        // Set Total Estimated Payment Variable
        this.payments = 0;

        // Set Most Recent Signup Variable
        const mostRecentTimeData = [];

        // Set Distinct Length Variables
        let distinctChurches = [];
        let distinctStates = [];

        // Set Count Variables
        this.numberOfServants = 0;
        this.numberOfMales = 0;
        this.numberOfFemales = 0;

        // Get Number of Feedback
        this.numberOfFeedback = 0;
        ...
        // ----------SECTION TWO: MASSIVE FOR EACH W/ CALCS-----------------

        attendees.forEach(attendee => {

            // Calculate Bar Chart Data
            const registerDate = new Date(attendee.time_registered);

            if (registerDate < dateOne) { barChartData[0]++; } else
            if (dateOne < registerDate && registerDate < dateTwo) { barChartData[1]++; } else
            if (dateTwo < registerDate && registerDate < dateThree) { barChartData[2]++; } else
            if (dateThree < registerDate && registerDate < dateFour) { barChartData[3]++; } else
            if (dateFour < registerDate && registerDate < dateFive) { barChartData[4]++; } else
            if (dateFive < registerDate && registerDate < dateSix) { barChartData[5]++; } else
            if (dateSix < registerDate && registerDate < dateSeven) { barChartData[6]++; } else
            if (dateSeven < registerDate && registerDate < dateEight) { barChartData[7]++; }
            // ----------------------------------------------------------


            // Calculate Age Group Data
            if (attendee.age <= 13) { this.ageGroupOne++; } else
            if (13 < attendee.age && attendee.age <= 17) { this.ageGroupTwo++; } else
            if (17 < attendee.age && attendee.age <= 21) { this.ageGroupThree++; } else
            if (21 < attendee.age) { this.ageGroupFour++; }
            // ----------------------------------------------------------


            // Calculate most recent time someone signed up
            mostRecentTimeData.push(new Date(attendee.time_registered));
            // ----------------------------------------------------------

            // Calculate estimated total payments
            const time_registered = new Date(attendee.time_registered);

            if (attendee.discount) {
                if (attendee.role && attendee.role === 'Staff') { this.payments = this.payments + 125; } else
                if (attendee.discount === 'ONEDAY2019') { this.payments = this.payments + 50; } else
                if (attendee.discount === '2019DAYSTWO') { this.payments = this.payments + 100; } else {
                    this.payments = this.payments + 135;
                }
            } else {
                if (attendee.time_registered && time_registered < this.lateFeeDate) { this.payments = this.payments + 155; } else {
                    this.payments = this.payments + 175;
                }
            }
            // ----------------------------------------------------------

            // Calculate length of churches and states
            distinctChurches.push(attendee.your_church);
            distinctStates.push(attendee.state);
            // ----------------------------------------------------------

            // Calculate count of servants and henders
            if (attendee.role && attendee.role !== Constants.ROLES[0]) { this.numberOfServants++; }
            if (attendee.gender === 'Male') { this.numberOfMales++; }
            if (attendee.gender === 'Female') { this.numberOfFemales++; }
            // ----------------------------------------------------------
        });


        // ----------SECTION THREE: FINAL CALCULATIONS-----------------

        // Most recent sign up calculations
        mostRecentTimeData.sort(function (a, b) {
            return a > b ? 1 : a < b ? -1 : 0;
        });

        const latestDate = (mostRecentTimeData[mostRecentTimeData.length - 1]).toString();

        const recentAttendee = attendees.find(attendee => {
            const attendeeTime = new Date(attendee.time_registered);
            const latest = new Date(latestDate);
            return attendeeTime.getTime() === latest.getTime();
        });
        this.recentSignUp = recentAttendee.first_name + ' ' + recentAttendee.last_name;
        // ----------------------------------------------------------


        // Calculate distinct number of churches
        distinctChurches = distinctChurches.filter((x, i, a) => a.indexOf(x) === i && x !== '' && x != null && x !== undefined).sort();
        this.numberOfChurches = distinctChurches.length;
        // ----------------------------------------------------------

        // Calculate distinct number of states
        distinctStates = distinctStates.filter((x, i, a) => a.indexOf(x) === i && x !== '' && x != null && x !== undefined).sort();
        this.numberOfStates = distinctStates.length;
        // ----------------------------------------------------------

        // Calculate bar chart data
        this.barChartData = [
            { data: barChartData, label: 'Registered' }
        ];
        // ----------------------------------------------------------
    }

    /**
     * load Google maps and heat map
     *
     */
    loadMap() {
